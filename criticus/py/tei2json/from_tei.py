...
def pre_parse_cleanup(text, regexes):
    text = re.sub(r" +|\t", " ", text)
    ...
    for regex in regexes:
        try:
            to_replace = unescape_string(regex.expression)
            replacement = unescape_string(regex.replacement)
            text = re.sub(to_replace, replacement, text)
            print(f"{to_replace=}\n{replacement=}")
        except Exception as e:
            return (False, f"Failed regular expression: {regex}\n{e}", "RegEx Fail")
    return text
