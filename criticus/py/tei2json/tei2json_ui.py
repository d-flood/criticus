...
def layout(settings: dict):
    if platform.system() == "Windows":
        ...
        output_folder_elem = sg.I(
            settings["ce_repo_dir"], key="output_dir", size=(30, 1)
        )
    ...
    input_frame = [
        [
            input_elem,
            sg.FileBrowse(
                initial_folder=settings["tei_dir"], file_types=(("XML Files", "*.xml"),)
            ),
        ],
        [sg.Radio("Convert All Verses", "all_or_one", key="all", enable_events=True)],
        [
            sg.Radio("Convert One Verse ", "all_or_one", key="one", enable_events=True),
            sg.T("Reference"),
            sg.I("", key="single_ref", disabled=True, enable_events=True),
        ],
        [
            sg.Text("Siglum Suffix (optional)"),
            sg.Input("", key="siglum_suffix", size=(5, 1)),
        ],
    ]
    output_frame = [
...
def enable_ui_elems(values: dict, window: sg.Window):
    ...
    if (values["all"] is False and values["one"] is False) or (
        values["one"] is True and values["single_ref"] == ""
    ):
            ...
            if values["tei_input"] == "" or values["output_dir"] == "":
                ...
                try:
                    result = t2j(
                        ...
                        values["output_dir"],
                        values["single_ref"],
                        values["siglum_suffix"],
                    )
                    if result:
                        cp.ok(
                            f"JSON transcription files saved to {values['output_dir']}",
                            "Success!",
                        )
                        print("success!")
                    else:
                    ...
                    cp.ok(
                        f"Conversion failed. Talk to David. See error below:\n{e}",
                        "Bummer...",
                    )
