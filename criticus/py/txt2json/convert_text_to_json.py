...
def build_witneses(siglum: str, tokens: list[dict]):
    return [{"id": siglum, "tokens": tokens}]
...
def construct_json_transcription(
    line: list[str], reference: str, siglum: str, output_dir
):
...
def convert_single_verse_to_json(
    text: str, siglum: str, reference: str, output_dir: str
):
    text = text.split()
...
def convert_text_to_json(
    ...
    for line in text:
        line = line.split()
        if (
            len(line) > 1 and line[0][0].isdigit() and line[0][-1].isdigit()
        ):  # check that line contains a reference and a text unit and is not a heading
            line, reference, verse = format_reference(line, reference_prefix)

            if (
                not convert_all and verse_to != verse_from
            ):  # handle a range of text units to convert
                print(f"{verse_from=} {verse_to=}")
                ...
                not convert_all and verse_from == verse_to
            ):  # handle a single text unit
                if convert_this_line(verse, verse_from, verse_to):
                    construct_json_transcription(line, reference, siglum, output_dir)

            else:
                construct_json_transcription(
                    line, reference, siglum, output_dir
                )  # handle all text units
                construct_json_transcription(
                    line, reference, siglum, output_dir
                )  # handle all text units
                construct_json_transcription(
