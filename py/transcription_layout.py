...
has not been set yet. Do this under 'Options'.")
            ...
            if cont == True:
                try:
                    ...
                    subprocess.call('start chrome "http://localhost:8000/wce-ote/"', shell=True)
                except:
                    messagebox.showerror('Uh-oh',
...
HTML to <your witnesses folder>/{ms}/OTE-code.')
                self.save_html_2(html, html_for_viewing, fname, ote_path, settings_file, ms)
...
Saved to OTE: {is_it_in_ote}')
    ...
    def combine_html(self):
        ...
        if settings_file['mss_path'] == '':
            messagebox.showerror('Uh-oh', message='Set witnesses directory in Settigs.')
        else:
            html_directory = fd.askdirectory(initialdir=settings_file['mss_path'])
...
the OTE for viewing a full document or converting to XML')
    ...
    def xml_to_txt(self):
        ...
        if mss_path != '':
            ...
            xml_fn = fd.askopenfilename(initialdir=mss_path, title='Select XML Transcription')
